...
void CalibrateJoy(int joynum)
{
    ...
    fontcolor = 13;
    CPPrint(gameStrings[STR_CalibrateJoy1]); // as: string replacements
    py += 6;
    fontcolor = 15;
    PPrint(gameStrings[STR_CalibrateJoy2]); // as: string replacements
    PPrint(gameStrings[STR_CalibrateJoy3]); // as: string replacements
    stage = 15;
    sx = (px + 7) / 8;
    ...
    py += 6;
    PPrint(gameStrings[STR_CalibrateJoy4]); // as: string replacements
    PPrint(gameStrings[STR_CalibrateJoy5]); // as: string replacements
    do				// wait for a button press
    {
        DrawChar(sx, py, stage);
        WaitVBL(3);
    ...
    py += 6;
    PPrint(gameStrings[STR_CalibrateJoy6]); // as: string replacements
    ch = PGet();
    if(ch == 'A' || ch == 'a')
        buttonflip = 1;
    else
...
void calibratekeys(void)
{
    ...
    ExpWin(22, 12);
    fontcolor = 13;
    CPPrint(gameStrings[STR_calibratekeys1]); // as: string replacements
    fontcolor = 15;
    PPrint(gameStrings[STR_calibratekeys2]); // as: string replacements
    PPrint(gameStrings[STR_calibratekeys3]); // as: string replacements
    PPrint(gameStrings[STR_calibratekeys4]); // as: string replacements
    PPrint(gameStrings[STR_calibratekeys5]); // as: string replacements
    PPrint(gameStrings[STR_calibratekeys6]); // as: string replacements
    PPrint(gameStrings[STR_calibratekeys7]); // as: string replacements
    PPrint(gameStrings[STR_calibratekeys8]); // as: string replacements
    hx = (px + 7) / 8;
    hy = py;
    for(i = 0; i < 4; i++)
    {
        px = pxl + 8 * 12;
        py = pyl + 10 * (1 + i);
        PPrint(gameStrings[STR_calibratekeys9]); // as: string replacements
        printscan(key[i * 2]);
    }
    px = pxl + 8 * 12;
    py = pyl + 10 * 5;
    ...
    do
    {
        ...
        select = ch - '1';
        DrawPchar(ch);
        PPrint(gameStrings[STR_calibratekeys10]); // as: string replacements
        ClearKeys();
        new = -1;
        while(!keydown[++new])
        if(new == 0x79)
...
unsigned InputInt(void)
{
    ...
    else if(string[0] == '%')
    {
        ...
        for(value = 0, loop1 = 0; loop1 <= digits; loop1++)
        {
            if(string[loop1 + 1]<'0' || string[loop1 + 1]>'1')
                return 0;
